...
NEWINCIDENT.ddlCrashType_vehicle = [{"Key":"Head on Collision","Value":"Head on Collision"},
									{"Key":"Collision with Parked Vehicle","Value":"Collision with Parked Vehicle"},
									{"Key":"Rear End Collision","Value":"Rear End Collision"},
									{"Key":"Collision while reversing","Value":"Collision while reversing"},
									{"Key":"Side Impact - Drivers Side","Value":"Side Impact - Drivers Side"},
									{"Key":"Side Impact - Passengers Side","Value":"Side Impact - Passengers Side"},
									{"Key":"Overturned","Value":"Overturned"},
									{"Key":"Struck Pedestrian","Value":"Struck Pedestrian"},
									{"Key":"Struck Animal","Value":"Struck Animal"},
									{"Key":"Struck Object","Value":"Struck Object"},
									{"Key":"Theft","Value":"Theft"}];
...
NEWINCIDENT.btnNavRight_click = function(e)
{
	...
	//alert("Before:" + NEWINCIDENT.activeBox);
	...
	//alert("After:" + NEWINCIDENT.activeBox);
...
NEWINCIDENT.setSessionValues = function(Category, Prefix)
{
	...
	{
		sessionStorage.parentPhotoPage = COMMON.SCREENS.NewIncident_Property; //Parent Page
...
NEWINCIDENT.readExistingIncident = function()
{
	...
	try
	{
		...
		if(incidentData.GPSLatitude == 'undefined' || incidentData.GPSLatitude =="")
			$("#lblGPSLatitude").text("No Location");
		else
			$("#lblGPSLatitude").text(incidentData.GPSLatitude);
		if(incidentData.GPSLongitude == 'undefined' || incidentData.GPSLongitude =="")
		...
		//NEWINCIDENT.setDropDownValue("ddlIsInjured", incidentData.IsInjured);
	}
	catch(e)
	{
		console.log(e);
...
NEWINCIDENT.saveExistingIncident = function()
{
	...
	try
	{
		...
		var selCrashType = "";
		if($("#ddlCrashType").val()!=null)
		{
			selCrashType = $("#ddlCrashType").val().join();
		}
		incidentData.CrashType = selCrashType; //$("#ddlCrashType option:selected").text();
		...
		//incidentData.IsInjured = $("#ddlIsInjured option:selected").text();      //New
...
$(document).ready(function()
{
	...
	if(false) //$("#ddlThirdPartyDamage option:selected").text().toUpperCase()=="YES")
	{
		$("#lblOtherDamageDetails").show();
		//$("#txtOtherDamageDetails").show();
	}
	...
	{
		if(false) //$("#ddlThirdPartyDamage").is(':visible'))
		{
	...
	COMMON.CurrentPage = COMMON.SCREENS.NewIncident_Property;
	
	//if(NEWINCIDENT.activeBox==1){$(".nav_left").hide();}	
	//if(NEWINCIDENT.activeBox==6){$(".nav_right").hide();}
